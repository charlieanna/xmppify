...
class RubyBOSH  
  ...
  def self.register(user)
    ...
    ap ENV['DOORKEEPER_APP_ID']
    encrypted_data = crypt.encrypt_and_sign("#{user.email}_#{ENV['DOORKEEPER_APP_ID']}") 
    jid    = Jabber::JID.new("#{encrypted_data}@idlecampus.com")
    client = Jabber::Client.new(jid)
    Jabber::debug = false 
    client.connect
    fields = {}
    client.register("#{ENV['DOORKEEPER_APP_SECRET']}", fields)
    client.close
    encrypted_data
  end
